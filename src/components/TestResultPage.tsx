...
const TestResultPage: React.FC = () => {
  ...
  const [userResult, setUserResult] = useState<Grade | null>(null);
    ...
    const fetchUserTestScore = async () => {
      try {
        ...
        if (!type || !id || type === "null" || id === "null") {
          setError("Missing course type or course ID in URL parameters");
          return;
        }
        ...
        if (response.data) {
          ...
          if (
            ...
            const result = getUserResult(response.data.score);
            setUserResult(result);
          }
        } else {
          setError("No test score data found");
        }
      } catch (err: any) {
        console.error(err);
  ...
  const getUserResult = (score: number): Grade | null => {
    return (
      grade.find((type) => score >= type.min_score && score <= type.max_score) ||
      null
    );
  };

  const getColorScheme = (grade: string) => {
    switch (grade) {
      case "F":
        return {
          bg: "bg-red-50",
  ...
  if (
    !userTestScore ||
    userTestScore.score === undefined ||
    userTestScore.score === null 
  ) {
    return (
      <div>
        <Header searchTerm={searchTerm} setSearchTerm={setSearchTerm} />
          ...
          <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center font-sans p-6 w-full">
            <div className="text-center">
              <p className="text-lg text-gray-600 mb-4">
                คุณยังไม่ได้ทำการทดสอบ
              </p>
              <a
                onClick={() => {
                  navigate(`/testPage?type=${type}&id=${id}`);
                }}
  ...
  const colors = userResult
    ? getColorScheme(userResult.name)
              ...
              >
                ...
                <div className="text-center">
                  ...
                  <h2 className={`text-2xl font-bold ${colors.title} mb-4`}>
                    ผลการทดสอบของคุณอยู่ในระดับ {userResult.name}
                  </h2>
                </div>
                ...
                <div className="flex justify-center space-x-4 mt-8">
                  <a
                    onClick={() => navigate(`/testPage?type=${type}&id=${id}`)}
